...
class CoveredCallTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        # Action space: [action_type, amount_ratio]
        # 0: BUY_STOCK, 1: SELL_STOCK, 2: HOLD, 3: SELL_COVERED_CALL, 4: CLOSE_COVERED_CALL
    ...
    def _map_action(self, action):
        ...
        if action_type == 0:  # BUY_STOCK
            ...
            return ("HOLD", 0)
        elif action_type == 3:  # SELL_COVERED_CALL
            # Use a fixed 30-day maturity and strike equal to current stock price
            maturity_dt = self.current_date_dt + timedelta(days=30)  # Use current_date_dt instead of parsing the string
            strike_price = float(current_price)
            portfolio_summary = self.portfolio.get_portfolio_summary(self.price_simulator, self.current_date_dt)
            available_shares = portfolio_summary["Stock Quantity"]
            max_contracts = available_shares  # assuming 1 share per contract
            ...
            # Look for an existing short call position to close
            open_calls = []
            for option_id, quantity in self.portfolio.current_holdings.items():
                if option_id.startswith("CALL_"):
                    if quantity < 0:  # Only consider short positions
    ...
    def step(self, action):
        ...
        # First close any covered calls expiring tomorrow
...
#             # Look for an existing short call position to close
#             open_calls = []
#             for option_id, holding in self.portfolio.current_holdings.items():
#                 if option_id.startswith("CALL_"):
#                     # Extract quantity if holding is a tuple, else use holding directly.
#                     qty = holding[0] if isinstance(holding, tuple) else holding
#                     if qty < 0:
#                         open_calls.append((option_id, qty))
...
#             strike, maturity_dt, qty = data
#             if qty > 0 and maturity_dt > self.current_date_dt:
#                 print("Selling covered call", strike, maturity_dt, qty, "at", self.current_date_dt)
#                 success = self.portfolio.sell_covered_call(self.price_simulator, self.current_date_dt, strike, maturity_dt, qty)
#         elif command == "CLOSE_COVERED_CALL":
#             strike, maturity_dt, qty = data
#             if qty > 0 and maturity_dt > self.current_date_dt:
#                 print("Closing covered call", strike, maturity_dt, qty, "at", self.current_date_dt)
#                 success = self.portfolio.buy_option(self.price_simulator, self.current_date_dt, "CALL", strike, maturity_dt, qty)
#         # HOLD: do nothing
...
#             "portfolio_return": portfolio_value_after / self.initial_portfolio_value - 1
#         }
#         print(f"Action: {action} obs: {observation} reward: {reward}")
